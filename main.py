...
from components.popup_color_item import PopupColorItem
from menu_data import GalleryData

logging.basicConfig(level=logging.INFO)

menuData = GalleryData()


async def main(page: ft.Page):
    page.title = "GNSWhatsSender"
    ...
    def find_control_group_object(control_group_name):
        for control_group in menuData.destinations_list:
    ...
    async def display_control_examples(control_group_name, control_id):
        control_group = find_control_group_object(control_group_name)
        rail.selected_index = menuData.destinations_list.index(control_group)
    ...
    async def control_group_selected(e):
        control_group_name = menuData.destinations_list[e.control.selected_index].name
        await page.go_async(f"/{control_group_name}")

    def get_destinations():
        destinations = []
        for destination in menuData.destinations_list:
            destinations.append(
    ...
    page.appbar = ft.AppBar(
        ...
        leading_width=40,
        title=ft.Text("GNS Message Sender"),
        center_title=True,
    ...
    print(f"Rota inicial: {page.route}")
